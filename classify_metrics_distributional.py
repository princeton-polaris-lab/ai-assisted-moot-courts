...
DATABASE_PATH = "data/distributional_metrics.db"
GET_CASES_QUERY =  "SELECT * FROM remark_transcript_context_1 WHERE remark_id IN (SELECT remark_id FROM distributional_metrics WHERE classification='');"
...
def metrics_main(cfg: DictConfig) -> None:
    ...
    # don't run metrics on generations without text
    cases_df = cases_df[cases_df["remark_text"] != ""]
